...
# PowerLevel10k prompt #
########################
# Load p10k only if NOT in eat (eat loads .p10k-eat.zsh instead)
...
###########################
# Eat-specific prompt fix #
###########################
# Use simplified prompt in eat, p10k in iTerm2
if [[ "$INSIDE_EMACS" == *"eat"* ]]; then
  # Load simplified prompt instead of p10k
  if [[ -f "${ZDOTDIR:-~}/.p10k-eat.zsh" ]]; then
    source "${ZDOTDIR:-~}/.p10k-eat.zsh"
  else
